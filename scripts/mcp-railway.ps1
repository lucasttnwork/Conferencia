# Carrega RAILWAY_TOKEN do .env (se existir) e inicia o MCP da Railway
...
Load-DotEnvVars -EnvFilePath $envPath -KeysToLoad @('RAILWAY_TOKEN')
...
Require-EnvVars -Names @('RAILWAY_TOKEN')

$npxPath = Resolve-NpxPath
if (-not $npxPath) { exit 1 }

& $npxPath -y @railway/mcp-server@latest 2>$null
