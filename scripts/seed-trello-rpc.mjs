		...
		cards.map((card) =>
			runLimited(async () => {
				...
				if (memberIds.length > 0) {
					// garantir que os membros existam e pertençam ao board do card
					await Promise.all(
						memberIds.map((mid) =>
							runLimited(async () => {
								await rpc('fn_upsert_member', {
	...
	await Promise.all(
		actions.map((a) =>
			runLimited(async () => {
				...
				if (card && card.id) {
					const cached = cardsById.get(card.id) || {};
					const merged = {
						id: card.id,
						// SEMPRE priorizar o board/list do action
						idBoard: (boardData && boardData.id) || cached.idBoard || null,
						idList: (listAfter && listAfter.id) || (listBefore && listBefore.id) || cached.idList || null,
						name: card.name ?? cached.name ?? null,
						desc: cached.desc ?? null,
						url: cached.url ?? null,
						closed: cached.closed ?? false,
						due: cached.due ?? null
					};
					cardsById.set(card.id, { ...cached, ...merged });
					await rpc('fn_upsert_card', {
						p_trello_id: merged.id,
						p_board_trello_id: merged.idBoard,
						p_current_list_trello_id: merged.idList,
						p_name: merged.name,
						p_description: merged.desc,
						p_url: merged.url,
						p_is_closed: !!merged.closed,
						p_due_at: merged.due
					});
				}
				...
				if (type === 'addMemberToCard' && a.data && a.data.member && card && card.id) {
					const effectiveBoardTid = (boardData && boardData.id) || (cardsById.get(card.id) && cardsById.get(card.id).idBoard) || null;
					if (effectiveBoardTid) {
						await rpc('fn_upsert_member', {
							p_trello_id: a.data.member.id,
							p_username: (a.data.member && a.data.member.username) || null,
							p_full_name: (a.data.member && a.data.member.fullName) || null,
							p_email: null
						});
						await rpc('fn_upsert_board_member', {
							p_board_trello_id: effectiveBoardTid,
							p_member_trello_id: a.data.member.id,
							p_role: null
						});
					}
					await rpc('fn_attach_card_members', {
...
async function fetchActionsLastDays(boardId, days) {
	...
	const maxActions = Number(process.env.TRELLO_MAX_ACTIONS || Infinity);
	while (true) {
		...
		before = last.date;
		if (all.length >= maxActions) break;
	}
	return all.filter((a) => {
...
async function backfillActionsLastDaysStream(boardId, days, cardsById) {
	const since = new Date(Date.now() - days * 24 * 60 * 60 * 1000);
	let before = null;
	let total = 0;
	const filter = 'updateCard:idList,createCard,addMemberToCard,removeMemberFromCard,addLabelToCard,removeLabelFromCard,updateCard';
	while (true) {
		...
		const last = batch[batch.length - 1];
		const lastDate = last && last.date ? new Date(last.date) : null;
		await processActions(batch, cardsById);
		total += batch.length;
		console.log(`Processados até agora: ${total} ações...`);
		if (!lastDate || lastDate <= since) break;
		before = last.date;
	}
	console.log(`Processamento de ações concluído. Total: ${total}`);
}
...
async function main() {
	...
	console.log('Processando ações (~60 dias) em lotes de até 1000...');
	await backfillActionsLastDaysStream(TRELLO_BOARD_ID, 60, cardsById);

	// Reconciliar cards atuais do board após ingestão das ações
	const cardsAfter = await trelloGet(`/boards/${TRELLO_BOARD_ID}/cards`, {
		fields: 'name,desc,url,idList,closed,due,idBoard,idMembers,idLabels',
		limit: 1000
	});
	await upsertCards(cardsAfter, board.id);
